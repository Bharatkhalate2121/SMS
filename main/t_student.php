...
?>
 ...
 <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
        ...
        <div class="table-responsive">
          ...
          <table class="table table-striped table-bordered" id="student_table">
            ...
            <tbody>
              ...
              <?php
              $t_id = $_SESSION['t_cid'];
              $q1 = "SELECT * FROM `students` WHERE `s_class` = '$t_id'";
              ...
              while ($result = mysqli_fetch_array($query1)) {
                ...
                <td>
                <button type="button" onclick="fun_view(' . $result["s_id"] . ')" class="btn btn-success mx-1" id="std' . $result["s_id"] . '">view</button>
                ...
                <td>
                <button type="button" onclick="fun_delete(' . $result["s_id"] . ')" class="btn btn-danger mx-1" id="std' . $result["s_id"] . '">Delete</button>
  ...
  <!-- view student info-->
   ...
   <!-- view-->
   <div class="modal fade" id="view_stu" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLabel">Student Details
          </h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div class="modal-body" id="uuser">
        <div class="modal-body" id="teacher_details">
				...
				<div class="col-md-3">
					<img src="" class="img-thumbnail" id="sv_image">
				</div>
				<div class="col-md-9">
					<table class="table">
						<tbody>
            <tr>
							<th>Name</th>
							<td id="sv_name">vishal gangji</td>
						</tr>
						<tr>
							<th>Roll No</th>
							<td id="sv_roll">solapur</td>
						</tr>
						<tr>
							<th>Gender</th>
							<td id="sv_gender">solapur</td>
						</tr>
						<tr>
							<th>DOB</th>
							<td id="sv_dob">solapur</td>
						</tr>
						<tr>
							<th>Age</th>
							<td id="sv_age">solapur</td>
						</tr>
						<tr>
							<th>Class</th>
							<td id="sv_class">cm - 1</td>
						</tr>
						<tr>
							<th>Mobile no</th>
							<td id="sv_mobno">cm - 1</td>
						</tr>
            <tr>
							<th>Email Address</th>
							<td id="sv_email">vishal@gmail.com</td>
						</tr>
						<tr>
							<th>Password</th>
							<td id="sv_password">diploma</td>
						</tr>
					</tbody></table>
          <div class="container">
<h2> Attendance</h2>
<div class="progress" style="height: 30px;">
  <div id="stu_attendance_width"class="progress-bar progress-bar-striped" role="progressbar" style="width: 50%" aria-valuenow="50" aria-valuemin="0" aria-valuemax="100">50px</div>
</div>
</div>';
				</div>
				</div></div>
...
function add() {
  ...
  function fun_view(idd) {
    ...
    $.ajax({
      ...
      dataType: "json",
      success: function(data) {
        $('#sv_id').val(data.teacher_id);
        $('#sv_name').html("");
        $('#sv_name').append(data.student_name);
        $('#sv_roll').html("");
        $('#sv_roll').append(data.student_roll_number);
        $('#sv_gender').html("");
        $('#sv_gender').append(data.student_gender);
        $('#sv_dob').html("");
        $('#sv_dob').append(data.student_dob);
        $('#sv_age').html("");
        $('#sv_age').append(data.student_age);
        $('#sv_mobno').html("");
        $('#sv_mobno').append(data.student_mobo);
        $('#sv_class').html("");
        $('#sv_class').append(data.student_class);
        $('#sv_address').html("");
        $('#sv_address').append(data.student_address);
        $('#sv_email').html("");
        $('#sv_email').append(data.student_email);
        $('#sv_password').html("");
        $('#sv_password').append(data.student_password);
        $("#sv_image").attr("src", data.student_photo);
        $("#stu_attendance_width").width(data.student_attendance);
        $("#stu_attendance_width").html("");
        $("#stu_attendance_width").append(data.student_attendance);
        // var percent_stu_att= document.getElementById('stu_attendance_width').style.width
