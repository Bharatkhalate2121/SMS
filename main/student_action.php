...
if($_POST["action"] == "edit_fetch")
	...
	if($_POST["action"] == "view_stu")
	{ $idd=$_POST["student_id"];
		$q1="SELECT * FROM students WHERE s_id='$idd'";
            $query1=mysqli_query($conn,$q1);
            while ($result=mysqli_fetch_array($query1)) {
				$c_id= $result['s_class'];
			$sql_class="SELECT * FROM `class` WHERE `id` =$c_id";
			...
			$stu_allatt_sql = "SELECT * FROM `attendance` WHERE student_id = '$idd' ";
			$result_stu_allatt=mysqli_query($conn,$stu_allatt_sql);
			$num_stu_allatt=mysqli_num_rows($result_stu_allatt);
			
			$stu_pre_sql="SELECT * FROM `attendance` WHERE `student_id` = '$idd' AND `attendance_status` = 'Present'";
			...
			$att_per =$num_stu_att/$num_stu_allatt*100;
				...
				$output["student_age"] =$from->diff($to)->y;
				$output["student_name"] = $result["s_name"];
				$output["student_roll_number"] =  $result["s_roll"];
				$output["student_attendance"] =  $att_per."%";
				$output["student_dob"] = $result["s_dob"];
				$output["student_class"] =  $row_class['name'];
				$output["student_id"] =  $result["s_id"];
				$output["student_gender"] = $result["s_gender"];
				$output["student_mobo"] =  $result["s_mobile"];
				$output["student_email"] = $result["s_email"];
